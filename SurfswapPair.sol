...
contract SurfswapPair is ISurfswapPair, SurfswapERC20 {
    ...
    uint40 public devFee = 5151515152; // uses 0.05% default from swap fee

    uint256 private unlocked = 1;
    modifier lock() {
        require(unlocked == 1, "Surfswap: LOCKED");
        unlocked = 0;
        _;
        unlocked = 1;
    }
    ...
    function initialize(address _token0, address _token1) external {
        require(msg.sender == factory, "Surfswap: FORBIDDEN"); // sufficient check
        token0 = _token0;
        token1 = _token1;
    }

    function setSwapFee(uint32 _swapFee) external {
        require(_swapFee > 0, "Surfswap: lower then 0");
        require(msg.sender == factory, "Surfswap: FORBIDDEN");
        require(_swapFee <= 1000, "Surfswap: FORBIDDEN_FEE");
        swapFee = _swapFee;
    }

    function setDevFee(uint32 _devFee) external {
        require(_devFee > 0, "Surfswap: lower then 0");
        require(msg.sender == factory, "Surfswap: FORBIDDEN");
        devFee = _devFee;
    }
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1)
        ...
        returns (bool feeOn)
    {
        address feeTo = ISurfswapFactory(factory).feeTo();
    ...
    function mint(address to) external lock returns (uint256 liquidity) {
        ...
        require(liquidity > 0, "Surfswap: INSUFFICIENT_LIQUIDITY_MINTED");
        _mint(to, liquidity);
        ...
        returns (uint256 amount0, uint256 amount1)
    {
        ...
        require(
            amount0 > 0 && amount1 > 0,
            "Surfswap: INSUFFICIENT_LIQUIDITY_BURNED"
        );
